...
var IA = {
	...
	Entity: function(name, x, y){
		this.name = name;
		this.x = x;
		this.y = y;

		this.addPosition = function(mX, mY){
			this.x += mX;
			this.y += mY;
...
var Multiplayer = {
	...
	client: function(host, port){
		...
		socket.on("packet", function(packet){
			_self.readPacket(JSON.parse(Util.decript(key, packet)));
		});
		socket.on("msg", function(text){
			_self.readMsg(Util.decript(key, text));
		});
	},
